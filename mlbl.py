...
class MLBL(object):
    ...
    def __init__(self,
        ...
        V: vocabulary size
        K: embedding dimensionality for words
    ...
    def init_params(self, embed_map, count_dict):
        ...
        if embed_map != None:
            R = np.zeros((self.K, self.V)) # Matrix R is K x V.
            for i in range(self.V): #For every word in vocab
                word = count_dict[i]
